    ...
    public class AdminController:BaseApiController
    {
        ...
        public async Task<ActionResult> EditRoles(string username,[FromQuery] string roles)
        {
            ...
            var result = await _userManager.AddToRolesAsync(
                user, selectedRoles.Except(userRoles)); //add user Roles

            if (!result.Succeeded)
                return BadRequest("Failed to add to roles");

            result = await _userManager.RemoveFromRolesAsync(user,
                userRoles.Except(selectedRoles));//remove user Roles

            if (!result.Succeeded)
                return BadRequest("Failed to remove the user roles");
        ...
        {
            return Ok("Only Moderation can see this");
        }
